 ...
 */
package org.cui.cockpits.setup;
...
import org.cui.cockpits.constants.CuiCockpitsConstants;
 ...
 */
@SystemSetup(extension = CuiCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(CuiCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(CuiCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						CuiCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/cuicockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
